...
case $RUNMOTHER in
	...
	ibm=0.0026
	python ecDiauxie.py -b ${ibm} -n ${maxsteps} -t 10 -x '-11.3' -s ${minstep} -p ${OUTDIR} --run --runconsortium --ratioecgl "1.0" --runglucose -l pfba_wphipsitrans_two -e '0.9' --phitransition --psitransition --kmtransphi "${kphi}" --kmtranspsi "${kpsi}" --vmaxpsi "${vpsi}" --vmaxphi "${vphi}" --phioffset "${phio}" --psioffset "${psio}"
...
### Sample mother cultures grown in M9G media and simulate daughter cultures in M9G and M9A media
	OUTDIRG=${OUTDIR}/M9G/
	mkdir -p ${OUTDIRG}
	python lagTimesFigure4.py -i "${OUTDIR}/endOfSimulation-ecoli_core-batch_low_Glc-pfba_wphipsitrans_two_ECgl_ECac_1p0.p" -p ${OUTDIRG} -S --kmtransphi "${kphi}" --kmtranspsi "${kpsi}" --vmaxpsi "${vpsi}" --vmaxphi "${vphi}" --phioffset "${phio}" --psioffset "${psio}" > ${OUTDIRG}/runsGlc.sh
...
case $PLOTLAGSIMU in
	...
	python lagTimesFigure4.py -i "${OUTDIR}/endOfSimulation-ecoli_core-batch_low_Glc-pfba_wphipsitrans_two_ECgl_ECac_1p0.p${x}" -p ${OUTDIR} -T
...
case $PLOTLAGPAPER in
	...
	x=""
	for f in ${OUTDIRG}/*p; do x="$x $f"; done
	python lagTimesFigure4.py -i "${OUTDIR}/endOfSimulation-ecoli_core-batch_low_Glc-pfba_wphipsitrans_two_ECgl_ECac_1p0.p${x}" -p ${OUTDIR} -T -U
...
case $PLOTLAGODE in
	...
	python lagTimesFigure4.py -i "${OUTDIR}/endOfSimulation-ecoli_core-batch_low_Glc-pfba_wphipsitrans_two_ECgl_ECac_1p0.p" -p ${OUTDIR} -T -O --kmtransphi "${kphi}" --kmtranspsi "${kpsi}" --vmaxpsi "${vpsi}" --vmaxphi "${vphi}" --phioffset "${phio}" --psioffset "${psio}" -U
	...
	python lagTimesFigure4.py -i "${OUTDIR}/endOfSimulation-ecoli_core-batch_low_Glc-pfba_wphipsitrans_two_ECgl_ECac_1p0.p" -p ${OUTDIR} -T -O --kmtransphi "${kphi}" --kmtranspsi "${kpsi}" --vmaxpsi "${vpsi}" --vmaxphi "${vphi}" --phioffset "${phio}" --psioffset "${psio}" -U -E
	...
	python lagTimesFigure4.py -i "${OUTDIR}/endOfSimulation-ecoli_core-batch_low_Glc-pfba_wphipsitrans_two_ECgl_ECac_1p0.p" -p ${OUTDIR} -T -O --kmtransphi "${kphi}" --kmtranspsi "${kpsi}" --vmaxpsi "${vpsi}" --vmaxphi "${vphi}" --phioffset "${phio}" --psioffset "${psio}" -U -N
	python lagTimesFigure4.py -i "${OUTDIR}/endOfSimulation-ecoli_core-batch_low_Glc-pfba_wphipsitrans_two_ECgl_ECac_1p0.p" -p ${OUTDIR} -T -O --kmtransphi "${kphi}" --kmtranspsi "${kpsi}" --vmaxpsi "${vpsi}" --vmaxphi "${vphi}" --phioffset "${phio}" --psioffset "${psio}" -U -E -N
