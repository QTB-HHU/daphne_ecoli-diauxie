...
ibm=0.0026
python ecDiauxie.py -b ${ibm} -n ${maxsteps} -t 10 -x '-11.5' -s ${minstep} -p ${OUTDIR}  --run --runconsortium --ratioecgl "1.0" --runglucose -l pfba_wphipsitrans_two -e '0.9' --phitransition --psitransition --kmtransphi "${kphi}" --kmtranspsi "${kpsi}" --vmaxpsi "${vpsi}" --vmaxphi "${vphi}" --phioffset "${phio}" --psioffset "${psio}"
...
python agreement.py -p ${OUTDIR} --runconsortium -m ${OUTDIR}/endOfSimulation-ecoli_core-fedbatch_low_Ac-pfba_wphipsitrans_two_ECgl_ECac_1p0.p
